...
(define-wiliki-action sitemap :read (_)
  `("Content-type: text/xml\n\n"
    "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
    "<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">\n"
    "<url>\n"
    "  <loc>" ,(wiliki:url :top) "</loc>\n"
    "  <changefreq>daily</changefreq>\n"
    "  <priority>1.0</priority>\n"
    "</url>\n"
    ,@(wiliki:db-fold
       (lambda (key val accum)
         (if (and (string? key) (#/^\d{8}[a-z]?-/ key))
             (let* ([page (wiliki:db-record->page (wiliki) key val)]
                    [mtime (~ page'mtime)]
                    [url #"~(wiliki:url :full)/~key"]
                    [lastmod (if mtime
                                 (sys-strftime "%Y-%m-%d" (sys-localtime mtime))
                                 #f)])
               (cons `("<url>\n"
                       "  <loc>" ,url "</loc>\n"
...
(define (clean-content-for-word-count content)
  (if (not (string? content))
      ""
      (let* ([t content]
             [t (regexp-replace-all #/\[\[\$\$.*?\]\]/ t "")]
             [t (regexp-replace-all #/(?:^|\n);;[^\n]*/ t "")]
             [t (regexp-replace-all #/```[a-zA-Z0-9]*\n?/ t "")]
             [t (regexp-replace-all #/!\[([^\]]*)\]\([^\)]+\)/ t "\\1")]
             [t (regexp-replace-all #/\[([^\]]+)\]\([^\)]+\)/ t "\\1")]
             [t (regexp-replace-all #/\[\[https?:\/\/[^\s\|]+\|([^\]]+)\]\]/ t "\\1")]
             [t (regexp-replace-all #/\[\[([^\]\|]+)\|https?:\/\/[^\s\]]+\]\]/ t "\\1")]
             [t (regexp-replace-all #/\[\[([^\]]+)\]\]/ t "\\1")]
             [t (regexp-replace-all #/\[https?:\/\/[^\s\]]+\s+([^\]]+)\]/ t "\\1")]
             [t (regexp-replace-all #/\[([^\|\]]+)\|https?:\/\/[^\s\]]+\]/ t "\\1")]
             [t (regexp-replace-all #/https?:\/\/[^\s<>\)]+/ t "")]
             [t (regexp-replace-all #/<[^>]+>/ t "")]
             [t (regexp-replace-all #/(?:^|\n)[ \t]*[#\*->\+\:]+[ \t]*/ t "\n")]
             [t (regexp-replace-all #/(?:^|\n)[ \t]*\d+\.[ \t]*/ t "\n")]
             [t (regexp-replace-all #/'''|''|"""|~~~|\*\*|\*|__|~%|~|`|\{\{\{|\}\}\}|\|\||\|/ t "")]
             [t (regexp-replace-all #/\s/ t "")])
        t)))
...
(define (entry-header entry :key (show-navi #t))
  ...
  (let1 key (index-entry-key entry)
    (let* ([page (wiliki:db-get key #f)]
           [content (if page (~ page 'content) "")]
           [cleaned (clean-content-for-word-count content)]
           [cl (number->string (exact (* (floor (/ (string-length cleaned) 100.0)) 100)))])
...
(define-reader-macro (amazon-affiliate asin . opts)
  (let-macro-keywords* opts ([title #f]
                             [image #f]
                             [price #f]
                             [rakuten #f])
    ...
    (if (and title image)
        (let1 url #"https://www.amazon.co.jp/dp/~|asin|?tag=~|aid|"
          `((div (@ (class "amazon-card"))
                 ...
                 (div (@ (class "amazon-card-info"))
                      ...
                      ,@(if price `((div (@ (class "amazon-card-price")) ,price)) '())
                      (div (@ (class "amazon-card-buttons"))
                           (a (@ (href ,url) (class "amazon-button") (target "_blank")) "Amazonで見る")
                           ,@(if rakuten
                                 `((a (@ (href ,rakuten) (class "amazon-button rakuten-button") (target "_blank")) "楽天で見る"))
                                 '()))))))
        #"<div class=\"iframely-embed\"><div class=\"iframely-responsive\" style=\"height: 250px !important; max-height: 250px !important; max-width: 500px; width: 100%; margin: 0 auto; padding-bottom: 0 !important; overflow: hidden;\">\
...
(define-method wiliki:format-head-elements ((fmt <blog-formatter>) page . opts)
  (append (next-method)
          (let1 json-ld (wiliki:generate-json-ld fmt page)
            (if json-ld
              `((script (@ (type "application/ld+json")) (stree ,json-ld)))
              '()))))

(define (wiliki:generate-json-ld fmt page)
  (let* ([w (wiliki)]
         [key (~ page'key)]
         [is-top (or (not key) (equal? key (~ w'top-page)))]
         [url (if is-top (wiliki:url :top) #"~(wiliki:url :full)/~key")]
         [author-name (~ w'author)]
         [blog-title (~ w'title)]
         [blog-desc (~ w'description)]
         [index-entry (get-index-entry key)]
         [page-title (wiliki:format-head-title fmt page)])
    (if index-entry
        ;; BlogPosting for individual entries
        (let* ([ymd (decompose-key key)]
               [pub-date (if ymd (apply format #f "~4,'0d-~2,'0d-~2,'0d" ymd) #f)]
               [mod-date (if (~ page'mtime)
                             (sys-strftime "%Y-%m-%dT%H:%M:%S%z" (sys-localtime (~ page'mtime)))
                             #f)]
               [desc (let ([content (~ page'content)])
                       (if (string? content)
                           (let* ([body (if (string-prefix? "* " content)
                                            (or (string-scan content "\n" 'after) "")
                                            content)]
                                  [plain (tree->string (wiliki-remove-markup body))]
                                  [len (string-length plain)])
                             (if (> len 100)
                                 (string-append (string-take plain 100) "...")
                                 plain))
                           #f))]
               [obj `(("@context" . "https://schema.org")
                      ("@type" . "BlogPosting")
                      ("mainEntityOfPage" . ,url)
                      ("headline" . ,(index-entry-title index-entry))
                      ,@(cond-list
                         [pub-date `("datePublished" . ,pub-date)]
                         [mod-date `("dateModified" . ,mod-date)]
                      ...
                      ("publisher" . (("@type" . "Organization")
                                      ...
                                      ("logo" . (("@type" . "ImageObject")
                                                 ("url" . ,#"~(wiliki:url :top)/assets/top.png"))))))])
          (construct-json-string obj))
        ;; Blog / WebSite for other pages (top page, archive, etc.)
        (let ([obj `(("@context" . "https://schema.org")
                     ("@type" . "Blog")
                     ("url" . ,url)
                     ("name" . ,(if is-top blog-title page-title))
